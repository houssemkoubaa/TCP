...
public class Frame_server extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        ta_chat.setBackground(new java.awt.Color(11, 1, 12));
    ...
    public void clientAdd(String dataset) {
        ...
        if (!users.contains(name)) {
            ...
            tellEveryone(done);  // Send completion message
        } else {
            ta_chat.append("User " + name + " is already connected.\n");
        }
    }
        ...
        public void run() {
            ...
            try {
                ...
                while (true) {
                    ...
                    // Read the client identifier (e.g., username)
                    String clientIdentifier = reader.readLine();
                    if (clientIdentifier != null && !clientIdentifier.isEmpty()) {
                        clientOutputStreams.add(writer);
                        clientAdd(clientIdentifier);  // Add the client identifier to users

                        // Start a new thread for handling client messages
                        Thread listener = new Thread(new ClientHandler(clientS, writer));
                        listener.start();

                        ta_chat.append("Got a connection from " + clientIdentifier + "\n");
                    }
                }
            } catch (Exception e) {
